...
class CtoFCriterion(nn.Module):
    ...
    def PCK(self, coord1, coord1_loop, weight, th=10):
        distance = torch.norm(coord1 - coord1_loop, dim=-1)
        distance_ = torch.zeros_like(distance)
        distance_[distance < th] = distance[distance < th]
        #distance_ = weight * distance_
        distance_flatten = distance_.reshape(-1)
        pck_metric = torch.count_nonzero(distance_flatten) / distance_flatten.numel()
        return pck_metric
    ...
    def forward(self, coord1, data, fmatrix, pose, im_size):
        ...
        pck = self.PCK(coord1, coord1_lf, weight_cycle_f)
        ...
        return loss, eloss_c, eloss_f, closs_c, closs_f, std_loss, pck
