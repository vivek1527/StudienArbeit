...
from Studienarbeit_corregistration.caps_implementation.CAPS.criterion import CtoFCriterion
from Studienarbeit_corregistration.caps_implementation.CAPS.network import CAPSNet
...
class CAPSModel():
    ...
    def backward_net(self):
        ...
        self.j_loss, self.eloss_c, self.eloss_f, self.closs_c, self.closs_f, self.std_loss, self.pck = loss
    ...
    def write_summary(self, writer, n_iter):
        ...
        global pckavg
        pckavg.append(self.pck.item())
        global lossavg
        lossavg.append(self.j_loss.item())
        print("%s | Step: %d, Loss: %2.5f" % (self.args.exp_name, n_iter, self.j_loss.item()))
        print("%s | Step: %d, PCK: %2.5f" % (self.args.exp_name, n_iter, self.pck.item()))
        ...
        if n_iter % self.args.log_scalar_interval == 0:
            ...
            writer.add_scalar('PCK', self.pck.item(), n_iter)
        ...
        if n_iter % 1000 == 0:
            writer.add_scalar('Mean-PCK', np.mean(pckavg))
            writer.add_scalar('Mean-loss', np.mean(lossavg))
            print("%s | Step: %d, Mean-PCK: %2.5f" % (self.args.exp_name, n_iter, np.mean(pckavg)))
            print("%s | Step: %d, Mean-loss: %2.5f" % (self.args.exp_name, n_iter, np.mean(lossavg)))
    ...
    def load_from_ckpt(self):
        ...
        if self.args.ckpt_path != "":
            ...
            ckpt_folder = os.path.join(self.args.outdir, 'feature_extraction')
